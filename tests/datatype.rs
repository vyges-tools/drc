...
fn space_rule_ignores_a_different_datatype_on_the_same_layer() {
    ...
    assert!(
        v.is_empty(),
        "66/20 spacing must not see the 66/44 contacts: {v:?}"
    );
    ...
    assert!(
        v44.iter().any(|v| v.value == 50),
        "66/44 spacing sees the 50-gap contact pair: {v44:?}"
    );

    // A bare layer number means datatype 0 — which has no shapes here, so nothing.
    let r0 = Rules::parse("space 66 210\n").unwrap();
    assert!(
        check_library(&l, Some("top"), &r0).unwrap().is_empty(),
        "bare 66 == 66/0, empty"
    );
}
...
fn width_rule_is_datatype_scoped() {
    ...
    assert!(
        check_library(&l, Some("top"), &r).unwrap().is_empty(),
        "width 66/20 ignores the 66/44 contact"
    );
